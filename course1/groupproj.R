...
# NA values present in column raised_amount_usd
 sum(is.na(master_frame$raised_amount_usd)) #19990
 
# not used-This gives as error for Decision_invest_type--Replace all NA values in raised amount column to 0
 #master_frame$raised_amount_usd[is.na(master_frame$raised_amount_usd)==T] <- 0
 
# not used--Check if there is any NA value remaining--
 #sum(is.na(master_frame$raised_amount_usd)) #0
 
# need to know Reason for using this-----summary for different types of funding
...
# RESULTS EXPECTED FOR COUNTRY 1-----new code needed-----

#1. Total number of investments(count)

#summarise(group_by(D1,main_sector), sum(total_number_investments,na.rm = TRUE))
sum(D1$total_number_investments)
...
#1. total number of investments (count)
sum(D1_merged$otal_number_investments)
#[1] 12012

#2. total amount of investment
sum(D1_merged$total_amount_invested)
#[1] 107318294664

#9. For the top sector count wise(point3) which company has received the highest investment?

C1_max<-subset(C1,C1$main_sector=="Others")
C1_max<-arrange(C1_max,desc(C1_max$raised_amount_usd))
View(C1_max)
length(which(C1_max$raised_amount_usd==15000000))

#10. For point 4 (second best sector count-wise), which company received the highest investment?
C1_max_2<-subset(C1,C1$main_sector=="Social..Finance..Analytics..Advertising")
C1_max_2<-arrange(C1_max_2,desc(C1_max_2$raised_amount_usd))
View(C1_max_2)
length(which(C1_max_2$raised_amount_usd==15000000))

# RESULTS EXPECTED FOR COUNTRY 2

#1. total number of investments
sum(D2_merged$Total_number_investments)

#2. total amount of investment
sum(D2_merged$total_amount_invested)

#9. For the top sector count wise(point3) which company has received the highest investment?

C2_max<-subset(C2,C2$main_sector=="Others")
C2_max<-arrange(C2_max,desc(C2_max$raised_amount_usd))
View(C2_max)
length(which(C2_max$raised_amount_usd==15000000))

#10. For point 4 (second best sector count-wise), which company received the highest investment?
C2_max_2<-subset(C2,C2$main_sector=="Social..Finance..Analytics..Advertising")
C2_max_2<-arrange(C2_max_2,desc(C2_max_2$raised_amount_usd))
View(C2_max_2)
length(which(C2_max_2$raised_amount_usd==15000000))

# RESULTS EXPECTED FOR COUNTRY 3

#1. total number of investments
sum(D3_merged$Total_number_investments)

#2. total amount of investment
sum(D3_merged$total_amount_invested)

#9. For the top sector count wise(point3) which company has received the highest investment?
C3_max<-subset(C3,C3$main_sector=="Others")
C3_max<-arrange(C3_max,desc(C3_max$raised_amount_usd))
View(C3_max)
length(which(C3_max$raised_amount_usd==15000000))

#10. For point 4 (second best sector count-wise), which company received the highest investment?
C3_max_2<-subset(C3,C3$main_sector=="Social..Finance..Analytics..Advertising")
C3_max_2<-arrange(C3_max_2,desc(C3_max_2$raised_amount_usd))
View(C3_max_2)
length(which(C3_max_2$raised_amount_usd==15000000))
